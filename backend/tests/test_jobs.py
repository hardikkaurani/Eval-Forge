...
def mock_session_local(db_session: AsyncSession):
    ...
    mock_callable = lambda: MockSessionContext(db_session)
    ...
    with (
        ...
        patch("app.jobs.executors.dataset_import.SessionLocal", mock_callable, create=True),
        patch("app.jobs.executors.dataset_export.SessionLocal", mock_callable, create=True),
...
def mock_celery_apply_async():
    ...
    with patch("app.jobs.services.job.run_background_job.apply_async", fake_apply_async_sync):
        yield
...
def test_jobs_lifecycle_endpoints(client: TestClient) -> None:
    ...
    create_response = client.post(f"/api/v1/jobs?project_id={project_id}", json=job_payload)
