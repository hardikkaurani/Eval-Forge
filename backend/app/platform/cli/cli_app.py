...
class EvalForgeCLI:
    ...
    def __init__(self):
        ...
        login_parser = subparsers.add_parser(
            "login", help="Authenticate with the EvalForge API"
        )
        ...
        eval_parser.add_argument(
            "--judge",
            required=True,
            choices=["geval", "deepeval", "rubric"],
            help="Judge algorithm",
        )
        eval_parser.add_argument(
            "--provider", required=True, help="LLM Provider (e.g. openai)"
        )
    ...
    def print_result(
        ...
        if use_json:
            print(
                json.dumps({"success": success, "message": message, "data": data or {}})
            )
            return
    ...
    def run(self, args: List[str]):
        ...
        if parsed.command == "login":
            ...
            if parsed.action == "create":
                self.print_result(
                    True,
                    f"Project '{parsed.name}' created.",
                    {"id": "proj_mock_12345", "name": parsed.name},
                    parsed.json,
                )
            else:
            ...
            if parsed.action == "upload":
                self.print_result(
                    ...
                    {"id": "ds_mock_67890", "file": parsed.file},
                    parsed.json,
                )
            else:
                self.parser.parse_args([parsed.command, "--help"])
            ...
            self.print_result(
                True,
                "Evaluation batch triggered.",
                {"run_id": "run_mock_abcde", "judge": parsed.judge},
                parsed.json,
            )
