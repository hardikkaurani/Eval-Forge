...
from app.advanced_ai.schemas import ConversationEvaluationCreate, ConversationEvaluationResponse
...
@router.post("", response_model=ConversationEvaluationResponse, status_code=status.HTTP_201_CREATED)
async def create_conversation_evaluation(
    payload: ConversationEvaluationCreate,
    db: AsyncSession = Depends(get_db)
):
    service = AgentConversationService(db)
    # Map turns list from payload schema details
    turns = payload.metrics_json.get("turns", [
