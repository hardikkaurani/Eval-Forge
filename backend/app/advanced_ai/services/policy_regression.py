...
from app.models.advanced_ai import Policy, RegressionRun, PromptVersion, RiskAssessment
from app.models.evaluation import EvaluationRun
from app.advanced_ai.repositories import AdvancedAIRepository
from app.advanced_ai.exceptions import PolicyNotFoundError, RegressionRunError
...
class PolicyRegressionService:
        ...
        for policy in policies:
            ...
            if rules.get("block_api_keys", False):
                if any(w in output_lower for w in ["sk-", "api_key", "bearer"]):
                    violations.append(f"Policy '{policy.name}' Violated: API key pattern detected.")

            # Rule: Never generate medical advice
            if rules.get("block_medical_advice", False):
                if any(w in output_lower for w in ["diagnose", "prescription", "medical advice"]):
                    violations.append(f"Policy '{policy.name}' Violated: Medical advice blocker triggered.")
            ...
            for topic in prohibited:
                if topic.lower() in prompt_lower or topic.lower() in output_lower:
                    violations.append(f"Policy '{policy.name}' Violated: Prohibited topic '{topic}' detected.")
    ...
    async def trigger_regression_check(
        ...
        try:
            ...
            stmt_compare = select(EvaluationRun).where(EvaluationRun.id == compare_run_id)
            ...
            raise RegressionRunError(f"Regression check execution failed: {str(e)}")
    ...
    def generate_ai_insights(self, safety_score: float, risk_score: float) -> List[Dict[str, str]]:
