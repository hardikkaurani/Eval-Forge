...
class DatasetRecordBase(BaseModel):
    ...
    input: Optional[str] = Field(None, description="Optional system prompt or input prefix.")
    context: Optional[str] = Field(None, description="Optional retrieval context or grounding information.")
    reference_output: Optional[str] = Field(None, description="Optional expected golden output.")
    candidate_output: Optional[str] = Field(None, description="Optional model output to evaluate.")
    ground_truth: Optional[str] = Field(None, description="Optional absolute ground truth for testing.")
    expected_score: Optional[float] = Field(None, description="Optional target evaluation score.")
...
class DatasetVersionBase(BaseModel):
    ...
    schema_version: str = Field("1.0", description="Version of the dataset schema format.")
    metadata_json: Dict[str, Any] = Field(default_factory=dict)
...
class DatasetBase(BaseModel):
    ...
    description: Optional[str] = Field(None, description="Detailed dataset explanation.")
    visibility: str = Field("private", description="Dataset visibility (public/private).")
    source: Optional[str] = Field(None, description="Data source or provenance details.")
