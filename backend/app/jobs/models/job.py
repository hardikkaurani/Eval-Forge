...
class Job(Base):
    ...
    execution_history: Mapped[List["ExecutionHistory"]] = relationship("ExecutionHistory", back_populates="job", cascade="all, delete-orphan")
    retry_history: Mapped[List["RetryHistory"]] = relationship("RetryHistory", back_populates="job", cascade="all, delete-orphan")
    cancellation: Mapped[Optional["Cancellation"]] = relationship("Cancellation", back_populates="job", uselist=False, cascade="all, delete-orphan")
...
class Worker(Base):
    ...
    last_heartbeat: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=get_utc_now, nullable=False)
...
class RetryHistory(Base):
    ...
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    attempted_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=get_utc_now, nullable=False)
...
class Cancellation(Base):
    ...
    cancelled_by: Mapped[str] = mapped_column(String(255), default="user", nullable=False)
    cancelled_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=get_utc_now, nullable=False)
