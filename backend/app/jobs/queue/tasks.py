...
async def async_run_background_job(job_id: str, celery_task_id: str) -> Dict[str, Any]:
    ...
    async with SessionLocal() as db:
        ...
        if job.status == "CANCELLED":
            await repo.add_job_log(job_id, "Job was cancelled before execution could begin.", "WARNING")
        ...
        await repo.add_job_log(job_id, f"Job execution started on worker task: {celery_task_id}")

        # Broadcast start event
        await websocket_manager.broadcast_job_update(job_id, {
            "event": "started",
            "job_id": job_id,
            "status": "RUNNING",
        ...
        async def progress_callback(progress: float, step: str) -> None:
            ...
            async with SessionLocal() as callback_db:
                ...
                await callback_repo.add_job_log(job_id, f"Progress: {progress:.1f}% - {step}")
        ...
        try:
            ...
            if job.retry_count < job.max_retries:
                ...
                await repo.record_retry(job_id, error_message=str(e), delay_seconds=delay_seconds)
                ...
                await repo.add_job_log(job_id, f"Job failed with error: {str(e)}. Scheduling retry attempt #{retry_attempt} in {delay_seconds} seconds.", "WARNING")
                ...
                await repo.add_job_log(job_id, f"Job execution failed permanently: {str(e)}", "ERROR")
