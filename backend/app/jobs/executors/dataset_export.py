...
class DatasetExportExecutor(BaseJobExecutor):
    ...
    async def execute(
        ...
        if not export_job_id or not version_id:
            raise ValueError(
                "Payload missing required 'export_job_id' or 'version_id' keys."
            )
        ...
        async with SessionLocal() as db:
            ...
            await progress_callback(
                50.0, "Serializing dataset structure to target format..."
            )

            saved_path = await service.execute_export(
                job_id=export_job_id, version_id=version_id
            )

            await progress_callback(
                100.0, f"Dataset export completed. File ready at: {saved_path}"
            )
            return {"export_job_id": export_job_id, "saved_path": saved_path}
