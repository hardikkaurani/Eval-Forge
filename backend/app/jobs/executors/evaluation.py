...
class EvaluationExecutor(BaseJobExecutor):
    ...
    async def execute(self, job: Job, progress_callback: ProgressCallback) -> Dict[str, Any]:
        ...
        async with SessionLocal() as db:
            ...
            await progress_callback(50.0, "Running judicial LLM evaluations (this may take a while)...")
            experiment = await service.execute_experiment(experiment_id)
            
            await progress_callback(100.0, "Evaluation pipeline successfully completed.")
            return {
