...
class AnalyticsAggregationExecutor(BaseJobExecutor):
    ...
    async def execute(self, job: Job, progress_callback: ProgressCallback) -> Dict[str, Any]:
        ...
        async with SessionLocal() as db:
            ...
            await progress_callback(40.0, "Computing averages, medians, latencies, and token costs...")
            
            snapshot = await service.compute_and_save_snapshot(project_id, scope, scope_id)
            
            await progress_callback(100.0, "Analytics snapshot aggregation successfully completed.")
            return {
