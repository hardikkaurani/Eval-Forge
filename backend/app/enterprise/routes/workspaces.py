...
from app.enterprise.schemas import WorkspaceCreate, WorkspaceResponse, QuotaResponse
...
async def create_workspace(payload: WorkspaceCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        org_id = uuid.UUID(payload.organization_id) if isinstance(payload.organization_id, str) else payload.organization_id
        ws = await workspace_service.create_workspace(db, org_id, payload.name, payload.description)
...
async def get_workspace_quota(
    ...
    status_data = await quota_service.check_quota_status(db, org_id, workspace_id, metric)
    return create_response(
        success=True,
        message="Workspace quota check completed.",
        data=status_data
    )
