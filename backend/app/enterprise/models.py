...
from sqlalchemy import Column, String, DateTime, Float, ForeignKey, Boolean, JSON, Integer
...
class Organization(Base):
    ...
    notifications = relationship("EnterpriseNotification", back_populates="organization", cascade="all, delete-orphan")
    api_keys = relationship("EnterpriseAPIKey", back_populates="organization", cascade="all, delete-orphan")
...
class Workspace(Base):
    ...
    usage_records = relationship("UsageRecord", back_populates="workspace", cascade="all, delete-orphan")
    quotas = relationship("Quota", back_populates="workspace", cascade="all, delete-orphan")
    audit_logs = relationship("AuditLog", back_populates="workspace", cascade="all, delete-orphan")
    api_keys = relationship("EnterpriseAPIKey", back_populates="workspace", cascade="all, delete-orphan")
...
class Role(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    organization_id = Column(UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=True)  # Null for system default roles
...
class Plan(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    name = Column(String(100), nullable=False, unique=True)  # Starter, Pro, Team, Business, Enterprise
    price_monthly = Column(Float, nullable=False)
    limits = Column(JSON, nullable=False)  # limits: api, storage, evaluation, concurrency, retention, etc.
    created_at = Column(DateTime, default=datetime.utcnow)
...
class UsageRecord(Base):
    ...
    organization_id = Column(UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), nullable=False)
    workspace_id = Column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=True)
    metric = Column(String(100), nullable=False)  # api_requests, evaluations, datasets, storage, bandwidth, etc.
...
class AuditLog(Base):
    ...
    workspace_id = Column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=True)
    user_id = Column(UUID(as_uuid=True), nullable=False)
    action = Column(String(100), nullable=False)  # e.g., login, logout, role_change, invitation_sent
...
class EnterpriseNotification(Base):
    ...
    channel = Column(String(50), nullable=False)  # email, slack, discord, webhook, in_app
