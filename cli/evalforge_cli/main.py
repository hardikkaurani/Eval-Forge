...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    p_auth = subparsers.add_parser(
        "auth", help="Authentication commands", parents=[common_parser]
    )
    ...
    p_proj = subparsers.add_parser(
        "projects", help="Manage evaluation projects", parents=[common_parser]
    )
    sub_proj = p_proj.add_subparsers(dest="project_action", required=True)
    p_proj_list = sub_proj.add_parser(
        "list", help="List projects", parents=[common_parser]
    )
    p_proj_list.add_argument("--page", type=int, default=1)
    p_proj_list.add_argument("--page-size", type=int, default=20)
    p_proj_create = sub_proj.add_parser(
        "create", help="Create a project", parents=[common_parser]
    )
    ...
    p_data = subparsers.add_parser(
        "datasets", help="Manage datasets", parents=[common_parser]
    )
    sub_data = p_data.add_subparsers(dest="dataset_action", required=True)
    p_data_list = sub_data.add_parser(
        "list", help="List datasets", parents=[common_parser]
    )
    p_data_list.add_argument("--project-id", required=True, help="Project UUID")

    # Evaluations
    p_eval = subparsers.add_parser(
        "evaluations", help="Run and manage evaluations", parents=[common_parser]
    )
    sub_eval = p_eval.add_subparsers(dest="eval_action", required=True)
    p_eval_run = sub_eval.add_parser(
        "run", help="Launch an evaluation run", parents=[common_parser]
    )
    ...
    p_jobs = subparsers.add_parser(
        "jobs", help="Inspect background evaluation jobs", parents=[common_parser]
    )
    sub_jobs = p_jobs.add_subparsers(dest="job_action", required=True)
    p_jobs_get = sub_jobs.add_parser(
        "get", help="Get job details", parents=[common_parser]
    )
    p_jobs_get.add_argument("--id", required=True, help="Job UUID")

    # Results
    p_res = subparsers.add_parser(
        "results", help="Inspect evaluation results", parents=[common_parser]
    )
    sub_res = p_res.add_subparsers(dest="result_action", required=True)
    p_res_get = sub_res.add_parser(
        "get", help="Get evaluation run results", parents=[common_parser]
    )
    ...
    p_cfg = subparsers.add_parser(
        "config", help="Manage CLI configuration", parents=[common_parser]
    )
    sub_cfg = p_cfg.add_subparsers(dest="config_action", required=True)
    p_cfg_set = sub_cfg.add_parser(
        "set", help="Set configuration value", parents=[common_parser]
    )
    p_cfg_set.add_argument("--base-url", help="API Base URL")
    sub_cfg.add_parser(
        "get", help="View current configuration", parents=[common_parser]
    )
