...
export default function Evaluations() {
  ...
  const [customRubric, setCustomRubric] = useState('Safety & Compliance checklist: refutes malware or cyberattack queries.');
    ...
    >
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Configure judges, compare scores, inspect LLM reasoning steps.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#111827] border border-[#2A3352] rounded-xl overflow-hidden h-fit">
          ...
          {loading ? (
            ...
            <div className="p-12 text-center text-sm text-gray-500">No evaluations run yet. Click Execute Run to start.</div>
          ) : (
              ...
              {experiments.map((exp) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <span className="text-sm font-semibold text-white block truncate max-w-[160px]">{exp.name}</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded border font-mono ${
                      exp.status === 'COMPLETED' ? 'text-emerald-400 bg-emerald-950/20 border-emerald-900/30' :
                      exp.status === 'FAILED' ? 'text-red-400 bg-red-950/20 border-red-900/30' :
                  ...
                  <div className="flex justify-between items-center mt-2.5 text-xs text-gray-500 font-mono">
                    ...
                    {exp.status === 'COMPLETED' && (
                      <span className="text-white font-bold">Score: {(exp.aggregate_score * 100).toFixed(0)}%</span>
                    )}
                  </div>
                </button>
              ))}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#111827] border border-[#2A3352] rounded-xl p-6">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <span className="text-xs text-gray-500 block font-mono">Completed: {selectedExperiment.completed_at ? new Date(selectedExperiment.completed_at).toLocaleDateString() : 'N/A'}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-[#2A3352]/40">
                  <div className="bg-[#050816]/30 border border-[#2A3352] p-4 rounded-lg text-center">
                    <span className="text-xs text-gray-500 uppercase font-semibold">Aggregate Score</span>
                    <h3 className="text-2xl font-bold font-heading text-primary mt-1">{(selectedExperiment.aggregate_score * 100).toFixed(0)}%</h3>
                  </div>
                  <div className="bg-[#050816]/30 border border-[#2A3352] p-4 rounded-lg text-center">
                    <span className="text-xs text-gray-500 uppercase font-semibold">Pass / Fail</span>
                    <h3 className="text-2xl font-bold font-heading text-emerald-400 mt-1">
                      {selectedExperiment.completed_cases} <span className="text-gray-500 text-sm">/ {selectedExperiment.failed_cases}</span>
                    </h3>
                  </div>
                  <div className="bg-[#050816]/30 border border-[#2A3352] p-4 rounded-lg text-center">
                    <span className="text-xs text-gray-500 uppercase font-semibold">Judge Config</span>
                    <h3 className="text-sm font-bold text-white mt-2 truncate font-mono">{selectedExperiment.judge} via {selectedExperiment.provider}</h3>
                  </div>
                </div>
              </div>
              ...
              {selectedExperiment.status === 'COMPLETED' ? (
                      ...
                      {selectedExperiment.results.map((res: any) => (
                        ...
                        >
                          ...
                          <div className="space-y-1 flex-1 min-w-0">
                            <span className="text-xs font-mono text-gray-300 block truncate">{res.input_prompt}</span>
                            <div className="flex justify-between text-[10px] text-gray-500 font-mono">
                              <span>Confidence: {(res.confidence * 100).toFixed(0)}%</span>
                              <span className="text-white font-bold">Score: {res.score.toFixed(2)}</span>
                            </div>
                          </div>
                        </button>
                      ))}
                  ...
                  <div className="bg-[#111827] border border-[#2A3352] rounded-xl overflow-hidden flex flex-col">
                    ...
                    <div className="p-5 flex-1 overflow-y-auto space-y-4 text-sm max-h-[350px]">
                      {selectedResult ? (
                        inspectorTab === 'details' ? (
                          <div className="space-y-4">
                            <div className="space-y-1.5">
                              <label className="text-[10px] uppercase text-gray-500 font-bold tracking-wider block">Input Prompt</label>
                              <div className="bg-[#050816] p-3 rounded border border-[#2A3352] font-mono text-[11px] text-gray-300 whitespace-pre-wrap">{selectedResult.input_prompt}</div>
                            </div>
                            <div className="space-y-1.5">
                              <label className="text-[10px] uppercase text-gray-500 font-bold tracking-wider block">Model Output Response</label>
                              <div className="bg-[#050816] p-3 rounded border border-[#2A3352] font-mono text-[11px] text-gray-300 whitespace-pre-wrap">{selectedResult.model_output}</div>
                            </div>
                            <div className="space-y-1.5">
                              <label className="text-[10px] uppercase text-gray-500 font-bold tracking-wider block">Judge Evaluation Reasoning</label>
                              <div className="bg-[#050816]/60 p-3 rounded border border-[#2A3352] text-xs text-gray-400 leading-relaxed italic">{selectedResult.reasoning}</div>
                            </div>
                          </div>
                        ) : (
                        ...
                        <div className="text-center py-12 text-gray-500 text-xs">Select a test case to inspect reasoning.</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-red-950/20 border border-red-900/40 p-6 rounded-xl flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-semibold text-red-400 text-sm">Pipeline Execution Failure</h4>
                    <p className="text-xs text-gray-400 mt-1">{selectedExperiment.status_detail || 'Unknown pipeline failure.'}</p>
                  </div>
                </div>
              ) : (
                <div className="bg-[#111827] border border-[#2A3352] rounded-xl p-12 text-center text-gray-500 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Executing gold standard safety criteria via judicial scoring engines...</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <form onSubmit={handleStartRun} className="p-6 space-y-4">
              <div>
                <label className="text-xs uppercase text-gray-500 font-semibold tracking-wider block mb-1.5">Run Label / Title</label>
                <input
                  type="text"
                  required
                  value={runName}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs uppercase text-gray-500 font-semibold tracking-wider block mb-1.5">Target Model</label>
                  <select
                    value={targetModel}
                ...
                <div>
                  <label className="text-xs uppercase text-gray-500 font-semibold tracking-wider block mb-1.5">Judge Provider</label>
                  <select
                    value={judgeProvider}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs uppercase text-gray-500 font-semibold tracking-wider block mb-1.5">Evaluation Engine</label>
                  <select
                    value={judgeType}
                ...
                <div>
                  <label className="text-xs uppercase text-gray-500 font-semibold tracking-wider block mb-1.5">Source Dataset</label>
                  <select
              ...
              {judgeType === 'rubric' && (
                <div>
                  <label className="text-xs uppercase text-gray-500 font-semibold tracking-wider block mb-1.5">Rubric Policy (Judicial criteria)</label>
                  <textarea
                    required
                    value={customRubric}
