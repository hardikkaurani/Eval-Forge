    ...
    >
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Cross-examine model scores, safety indices, retrieval leaderboards.
          </p>
        </div>
        <button
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Model scoring breakdown across safety, SQL generation, context, and formatting.
              </p>
            </div>
          ...
          <div>
            ...
            <div className="space-y-3.5">
              {[
                ...
                <div
                  key={model.rank}
                  className="flex justify-between items-center bg-[#050816]/30 border border-[#2A3352] p-3 rounded-lg text-xs"
                >
      ...
      <div className="bg-[#111827] border border-[#2A3352] rounded-xl overflow-hidden">
        <div className="px-6 py-5 border-b border-[#2A3352] bg-[#050816]/30">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            List of active benchmark definitions in this project.
          </p>
        </div>
        {loading ? (
          ...
          <div className="p-12 text-center text-sm text-gray-500">
            No benchmark suites defined. Click New Suite to define one.
          </div>
        ) : (
          <div className="divide-y divide-[#2A3352]">
            {suites.map((suite) => (
              <div
                key={suite.id}
                className="p-6 flex justify-between items-center hover:bg-[#1f2937]/20 transition-colors"
              >
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    {suite.tags.map((tag: string) => (
                      <span
                        key={tag}
                        className="text-[9px] uppercase font-bold tracking-wider text-cyan-400 bg-cyan-950/20 px-2 py-0.5 rounded border border-cyan-800/30"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="text-right text-xs text-gray-500 font-mono space-y-1">
                  <div>
                    Datasets Linked:{' '}
                    <span className="text-white font-bold">{suite.datasets_count}</span>
                  </div>
                  <div>
                    Created:{' '}
              ...
              <div>
                ...
                <div className="space-y-2 max-h-36 overflow-y-auto border border-[#2A3352] rounded-lg p-3 bg-[#050816]/40">
                  {datasets.map((ds) => (
                    <label
                      key={ds.id}
                      className="flex items-center gap-2.5 text-xs cursor-pointer text-gray-300 hover:text-white"
                    >
                      <input
                        type="checkbox"
